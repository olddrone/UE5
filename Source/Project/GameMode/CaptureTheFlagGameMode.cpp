// Fill out your copyright notice in the Description page of Project Settings.

#include "CaptureTheFlagGameMode.h"
#include "Project/Weapon/Flag.h"
#include "Project/CaptureTheFlag/FlagZone.h"
#include "Project/GameState/BlasterGameState.h"

void ACaptureTheFlagGameMode::PlayerEliminated(ABlasterCharacter* EliminatedCharacter,
	ABlasterPlayerController* VictimController, ABlasterPlayerController* AttackController)
{
	ABlasterGameMode::PlayerEliminated(EliminatedCharacter, VictimController, AttackController);
}

void ACaptureTheFlagGameMode::FlagCaptured(AFlag* Flag, AFlagZone* Zone)
{
	bool bValidCapture = (Flag->GetTeam() != Zone->Team);
	ABlasterGameState* BGameState = Cast<ABlasterGameState>(GameState);
	if (BGameState)
	{
		if (Zone->Team == ETeam::ET_BlueTeam)
			BGameState->BlueTeamScores();
		if (Zone->Team == ETeam::ET_RedTeam)
			BGameState->RedTeamScores();
	}
}
