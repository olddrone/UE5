...
#include "Project/BlasterTypes/Announcement.h"
...
void ABlasterPlayerController::ClientJoinMidgame_Implementation(FName StateOfMatch, 
	float Warmup, float Match, float Cooldown, float StartingTime)
{
	WarmupTime = Warmup;
	MatchTime = Match;
		...
		if (bHUDValid)
		{
			...
			if (BlasterGameState && BlasterPlayerState)
			{
				...
				FString InfoTextString = (bShowTeamScores)
					?GetTeamsInfoText(BlasterGameState) : GetInfoText(TopPlayers);
